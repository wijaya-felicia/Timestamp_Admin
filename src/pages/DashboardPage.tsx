    ...
    const fetch = async () => {
      await _booth
        ...
        .then((response) => {
          ...
          setUrgentBooths(response.filter((booth) => booth.status === 0));
          setWarningBooths(response.filter((booth) => booth.status === 1));
          setActiveBooths(response.filter((booth) => booth.status >= 2));
        })
        .catch((error) => {
          handleError(error);
        });
    ...
    <>
      <Overflow height="calc(100vh - 90px)">
        <div className="d-flex flex-column align-items-center py-3 gap-5 h-100">
          <OverflowX width="calc(100vw - 137px)">
            <div className="d-flex w-100 align-items-stretch justify-content-evenly">
              <div className="col px-3">
                <a
                  href="/booths"
                  className="card text-white text-decoration-none w-100 h-100"
                >
                  <div className="card-body text-center d-flex flex-column justify-content-center">
                    <h3 className="card-title text-nowrap">Manage Booths</h3>
                    <p className="card-text text-nowrap fs-5">
                      {boothCount} Booths Available
                    </p>
                  </div>
                </a>
              </div>
          ...
          <div
            className="d-flex flex-column flex-lg-row px-3 w-100 gap-3"
                  ...
                  <ul className="list-group list-group-flush">
                    {urgentBooths.map((booth) => (
                      <a
                        ...
                        key={booth.id}
                        className="list-group-item bg-tertiary d-flex justify-content-between align-items-center"
                      >
                        <span className="fs-4">{booth.name}</span>
                        <span className="badge bg-danger rounded-pill fs-5">
                          Error
                        </span>
                      </a>
                    ))}
                    {warningBooths.map((booth) => (
                      ...
                      >
                        <span className="fs-4">{booth.name}</span>
                        <span className="badge bg-warning rounded-pill fs-5">
                          Warning
                        </span>
                      </a>
                    ))}
                ...
                <Overflow height="calc(100vh - 376px)">
                  <ul className="list-group list-group-flush">
                    {adminLogs.map((log) => (
                      <li
                        key={log.id}
                        className="list-group-item bg-tertiary d-flex justify-content-between align-items-center gap-3"
                      >
                        <div className="d-flex gap-4">
                          <div className="d-flex align-items-center gap-2">
                            <Icon type="person" />
                            <span className="fs-5 fw-bold">
                              {getUser(log.userId)?.name}
                            </span>
                          </div>
                          <span className="fs-5">{log.message}</span>
                        </div>
                        <span className="fs-6 text-muted text-nowrap">
